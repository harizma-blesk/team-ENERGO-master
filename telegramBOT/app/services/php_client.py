...
class PhpClient:
    def __init__(self, settings: Settings) -> None:
        ...
        self._client = httpx.AsyncClient(
            base_url=settings.php_base_url.rstrip("/"),
            timeout=timeout,
    ...
    def _auth_headers(self) -> dict[str, str]:
        ...
        secret = self.settings.php_auth_secret
        scheme = self.settings.php_auth_scheme
        if not secret or scheme == "none":
            return headers
        if scheme == "api_key":
            headers[self.settings.php_api_key_header] = secret
            return headers
        if scheme == "bearer":
        ...
        *,
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                logger.info(
                    "php_request: %s %s attempt=%d payload=%s",
            ...
            if response.status_code >= 400:
                ...
                raise PhpClientError(
                    "PHP API returned an error response.",
                    status_code=response.status_code,
                    details=details,
            ...
            try:
                ...
                logger.error(
                    "php_non_json_response",
                    extra={"status_code": response.status_code, "path": path},
                )
                raise PhpClientError(
                    "PHP API returned non-JSON body.",
                    status_code=response.status_code,
                    details=response.text[:500],
                ) from exc
            ...
            if not isinstance(body, dict):
                raise PhpClientError(
                    "PHP API returned unexpected JSON format.",
                    status_code=response.status_code,
                    details=body,
                )
            return body
        ...
        logger.error(
            "php_unavailable_after_retries",
            extra={"path": path, "retries": retries, "error": str(last_error)},
        )
        raise PhpClientError(
            "PHP API is unavailable after retries.",
    ...
    async def cancel_booking(self, payload: dict[str, Any]) -> dict[str, Any]:
        """Call PHP API to cancel a booking (delete from DB)."""
